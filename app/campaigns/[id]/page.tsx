...
export default function CampaignWorkspace({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="min-h-[100dvh] bg-background"><div className="max-w-6xl mx-auto p-4 sm:p-6 lg:p-8 space-y-5">
    <header className="space-y-4"><Button asChild variant="ghost" size="sm" className="-ml-3"><Link href="/campaigns"><ArrowLeft className="w-4 h-4 mr-1" />Campaigns</Link></Button><div className="flex flex-wrap justify-between gap-4"><div className="min-w-0"><div className="flex flex-wrap gap-2 items-center"><StatusBadge status={campaign.status} /><span className="font-mono text-xs text-muted-foreground">Campaign {campaign.publicId}</span></div><h1 className="mt-2 font-serif text-3xl font-semibold tracking-tight sm:text-[34px]">{campaign.name}</h1><p className="mt-1 text-sm text-muted-foreground">{campaign.companyName} · <span className="break-all">{campaign.businessUrl}</span></p><p className="mt-3 max-w-2xl text-sm">Your campaign work, approvals, and export-ready assets live here. Review the checks, then choose the next move.</p></div><div className="flex w-full gap-2 sm:w-auto sm:items-start"><Button variant="outline" onClick={exportCampaign} data-testid="button-export-campaign" className="min-h-11 flex-1 sm:flex-none"><Download className="w-4 h-4 mr-2" />Export</Button><Button type="button" onClick={() => router.push(`/dashboard?campaignId=${encodeURIComponent(campaignId(campaign))}`)} data-testid="button-open-dashboard" className="min-h-11 flex-1 sm:flex-none">Create title pool <ExternalLink className="w-4 h-4 ml-2" /></Button></div></div></header>
    ...
    <div className="border-t border-[#5B7F6F]/25 pt-8"><AdsLab campaign={campaign} /></div>
  </div></div>;
}