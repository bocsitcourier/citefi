    ...
    return await withAuthenticatedTeamContext(request, async ({ teamId }) => {
      ...
      if (source.sourceType === "article_asset") {
        const [asset] = await db.select({ teamId: articleAssets.teamId })
          .from(articleAssets)
          .where(eq(articleAssets.id, source.sourceId))
          .limit(1);
        if (!asset || asset.teamId !== teamId) {
          return NextResponse.json({ error: "Asset not found" }, { status: 404 });
        }
        const { POST: regenerateArticleImage } = await import("@/app/api/media/[id]/regenerate/route");
        return regenerateArticleImage(request, {
          params: Promise.resolve({ id: String(source.sourceId) }),
        });
      }
      ...
      if (source.sourceType === "social_asset") {
        const [asset] = await db.select({ socialPostId: socialPostAssets.socialPostId })
          ...
          .where(eq(socialPostAssets.id, source.sourceId))
          .limit(1);
